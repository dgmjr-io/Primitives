...
#endif
{
  ...
  static iri IStringWithRegexValueObject<iri>.ExampleValue =>
      new(ExampleStringValue);
#else
readonly string IStringWithRegexValueObject<iri>.Description => Description;
readonly iri IStringWithRegexValueObject<iri>.ExampleValue =>
    ExampleStringValue;
...
//     // static iri IUriConvertible<iri>.FromUri(Uri iri) =>
//     From(urn.ToString());
// #else
...
public static iri
FromUri(Uri iri) => From(iri) with { OriginalString = iri.ToString() };

public static iri
From(string s) => Validate(s) == Validation.Ok
                      ? new iri(s) with { OriginalString = s }
                      : Empty;
...
operator iri(string s) => From(s) with { OriginalString = s };

public static implicit operator string(iri iri) => iri.ToString();

public static bool
operator ==(iri? left, IResourceIdentifier right) => left?.CompareTo(right) ==
                                                     0;
...
public static bool
operator <(iri? left, IResourceIdentifier right) => left?.CompareTo(right) < 0;
...
public static bool TryParse(string? s, IFormatProvider? formatProvider,
                            out iri iri) => TryParse(s, out iri);
...
{
    public override iri Read(ref Utf8JsonReader reader, type typeToConvert,
                             Jso options)
    {
...
public static class IriEfCoreExtensions {
  public static void
  ConfigureIri<TEntity>(this ModelBuilder modelBuilder,
  ...
  public static void ConfigureIri<TEntity>(this EntityTypeBuilder<TEntity> entityBuilder,
  Expression<Func<TEntity, iri>> propertyExpression)
      where TEntity : class => entityBuilder.Property(propertyExpression).HasConversion<iri.EfCoreValueConverter>(); }
#endif
