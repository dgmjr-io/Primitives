...
public partial record struct PhoneNumber
    : IStringWithRegexValueObject<PhoneNumber>
{
    /// <summary>
    ...
    FromUri(string s) => From(s.Remove(0, UriPrefix.Length)) with
    {
        OriginalString = s.Remove(0, UriPrefix.Length)
    };

    public static PhoneNumber FromUri(Uri u) => FromUri(u.ToString()) with
    {
        OriginalString = u.ToString()
    };

#if NET6_0_OR_GREATER
  static string IStringWithRegexValueObject<PhoneNumber>.RegexString =>
      RegexString;
  static string IStringWithRegexValueObject<PhoneNumber>.Description =>
      Description;
#else
    readonly string IStringWithRegexValueObject<PhoneNumber>.RegexString =>
        RegexString;
    readonly string IStringWithRegexValueObject<PhoneNumber>.Description =>
        Description;
    readonly PhoneNumber IStringWithRegexValueObject<PhoneNumber>.ExampleValue =>
        ExampleValue;
#endif

    public override readonly string ToString() =>
        IsEmpty ? string.Empty
                : _util.Format(ParsedNumber, PhoneNumberFormat.E164);
    ...
    private const Rxo RegexOptions = Compiled | CultureInvariant | IgnoreCase |
  ...
  // Regex IStringWithRegexValueObject<PhoneNumber>.RegexString => Regex();
    ...
    operator PhoneNumber?(string? s) => TryParse(s, out var number) ? number
                                                                    : default;

    public static implicit operator string?(PhoneNumber? n) =>
        n.HasValue ? _util.Format(n.Value.ParsedNumber, PhoneNumberFormat.E164)
                   : default;
  ...
  Validate(string s) => Util.IsViablePhoneNumber(s) && Regex().IsMatch(s)
                            ? Validation.Ok
                            : Validation.Invalid("Phone number is not valid.");
#else
    public static Validation Validate(string s) =>
        Util.IsViablePhoneNumber(s) &&
                ((IStringWithRegexValueObject<PhoneNumber>)ExampleValue)
                    .Regex()
                    .IsMatch(s)
            ? Validation.Ok
            : Validation.Invalid("Phone number is not valid.");
    ...
    public static PhoneNumber
    Parse(string value) => From(value) with { OriginalString = value };

    public readonly int CompareTo(object? obj) => obj is not PhoneNumber n
                                                      ? -1
    ...
    public static PhoneNumber From(string s) =>
        Validate(s) == Validation.Ok
            ...
            {
                Value =
                                      _util.Parse(s, DefaultRegion).ToString(),
                OriginalString = s
            }
    ...
    public readonly int
    CompareTo(PhoneNumber other) => CompareOrdinal(Value, other.Value);
#endif
...
public static class PhoneNumberEfCoreExtensions
{
    public static void ConfigurePhoneNumber<TEntity>(
        ...
        where TEntity : class => modelBuilder.Entity<TEntity>().ConfigurePhoneNumber(propertyExpression);

    public static void ConfigurePhoneNumber<TEntity>(this EntityTypeBuilder<TEntity> entityBuilder,
    Expression<Func<TEntity, PhoneNumber>> propertyExpression)
        where TEntity : class => entityBuilder.Property(propertyExpression).HasConversion<PhoneNumber.EfCoreValueConverter>();
}

  //"^\+((?:\+|00)[17](?: |\-)?|(?:\+|00)[1-9]\d{0,2}(?:
  //|\-)?|(?:\+|00)1\-\d{3}(?: |\-)?)?(0\d|\([0-9]{3}\)|[1-9]{0,3})(?:((?:
  //|\-)[0-9]{2}){4}|((?:[0-9]{2}){4})|((?: |\-)[0-9]{3}(?:
  //|\-)[0-9]{4})|([0-9]{7}))$"
