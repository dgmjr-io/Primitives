...
#endif
{
  ...
  static xri IStringWithRegexValueObject<xri>.ExampleValue =>
      ExampleStringValue;
#else
  readonly string IStringWithRegexValueObject<xri>.Description => Description;
  readonly xri IStringWithRegexValueObject<xri>.ExampleValue =>
      ExampleStringValue;
  ...
  public static xri Parse(string s,
  ...
  public static bool TryCreate(string? urlString, UriKind? uriKind,
    ...
    if (Validate(urlString) == Validation.Ok &&
  ...
  From(string s) => Validate(s) == Validation.Ok
                        ? new xri(s) with { OriginalString = s }
                        : Empty;

  public static xri
  From(Uri xri) => new xri(xri) with { OriginalString = xri.ToString() };
  ...
  operator xri(string s) => From(s) with { OriginalString = s };

  public static implicit operator string(xri xri) => xri.ToString();

  public static bool
  operator ==(xri? left, IResourceIdentifier right) => left?.CompareTo(right) ==
                                                       0;
  ...
  public static bool
  operator<(xri? left, IResourceIdentifier right) => left?.CompareTo(right) < 0;
  ...
  public static bool TryParse(string? s, IFormatProvider? formatProvider,
                              out xri xri) => TryParse(s, out xri);
  ...
  public class EfCoreValueConverter
      : Microsoft.EntityFrameworkCore.Storage.ValueConversion
            .ValueConverter<xri, string> {
    public EfCoreValueConverter() : base(v => v.ToString(), v => From(v)) {}
  }

  public class JsonConverter : JsonConverter<xri> {
    public override xri Read(ref Utf8JsonReader reader, Type typeToConvert,
                             Jso options) => From(reader.GetString());
...
public static class xriEfCoreExtensions {
  public static void
  ConfigureXri<TEntity>(this ModelBuilder modelBuilder,
  ...
  public static void ConfigureXri<TEntity>(this EntityTypeBuilder<TEntity> entityBuilder,
  Expression<Func<TEntity, xri>> propertyExpression)
      where TEntity : class => entityBuilder.Property(propertyExpression).HasConversion<xri.EfCoreValueConverter>(); }
