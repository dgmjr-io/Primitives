...
#endif
public readonly partial record struct uri : IStringWithRegexValueObject<uri>,
                                            IResourceIdentifier
...
#endif
{
  ...
  static uri IStringWithRegexValueObject<uri>.ExampleValue =>
      new(ExampleStringValue);

  static uri IStringWithRegexValueObject<uri>.Parse(string s) => From(s) with {
    OriginalString = s
  };
#else
  ...
  public class JsonConverter : JsonConverter<uri> {
    public override uri Read(ref Utf8JsonReader reader, type typeToConvert,
...
public static class UriEfCoreExtensions {
  public static void
  ConfigureUri<TEntity>(this ModelBuilder modelBuilder,
  ...
  public static void ConfigureUri<TEntity>(this EntityTypeBuilder<TEntity> entityBuilder,
  Expression<Func<TEntity, uri>> propertyExpression)
      where TEntity : class => entityBuilder.Property(propertyExpression).HasConversion<uri.EfCoreValueConverter>(); }
#endif
