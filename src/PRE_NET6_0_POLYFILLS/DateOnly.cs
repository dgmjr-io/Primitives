...
namespace System
{
                                ...
                                //IUtf8SpanFormattable
    {
        ...
        public DateOnly(int year, int month, int day, Calendar calendar) => dt = new DateTime(year, month, day, calendar);
        ...
        public DateTime ToDateTime(TimeOnly time) => dt.AddHours(time.Hour).AddMinutes(time.Minute).AddSeconds(time.Second).AddMilliseconds(time.Millisecond).AddMicroseconds(time.Microsecond);
        ...
        /// <returns>The DateTime instance composed of the date of the current DateOnly instance and the time specified by the input time.</returns>
        public DateTime ToDateTime(TimeOnly time, DateTimeKind kind) => dt.AddHours(time.Hour).AddMinutes(time.Minute).AddSeconds(time.Second).AddMilliseconds(time.Millisecond).AddMicroseconds(time.Microsecond).OfKind(kind);
        ...
        public int CompareTo(object? value)
        {
            ...
            {
                throw new ArgumentException($"The argument '{value}' was not recognized as a valid {nameof(DateOnly)}.", nameof(value));
            }
        ...
        public static DateOnly Parse(string s, string format) => DateTime.ParseExact(s, format, null).Date;
    }
}
