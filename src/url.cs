...
#endif
{
  ...
  static url IStringWithRegexValueObject<url>.ExampleValue =>
      ExampleStringValue;
#else
readonly string IStringWithRegexValueObject<url>.Description => Description;
readonly url IStringWithRegexValueObject<url>.ExampleValue =>
    ExampleStringValue;
...
public static url FromUri(url url) => From(url.ToString()) with
{
...
public static url Parse(string s,
                        IFormatProvider? formatProvider = null) => From(s);
...
public static bool TryCreate(string? urlString, UriKind? uriKind,
                             out url url) => TryParse(urlString, out url);

public static url
From(string s) => Validate(s) == Validation.Ok
                      ? new url(s) with { OriginalString = s }
                      : Empty;

public static url From(url url) => new url(url.ToString()) with
{
    OriginalString = url.ToString()
};
...
operator url(string s) => From(s) with { OriginalString = s };

public static implicit operator string(url url) => url.ToString();

public static bool
operator ==(url? left, IResourceIdentifier right) => left?.CompareTo(right) ==
                                                     0;
...
public static bool
operator <(url? left, IResourceIdentifier right) => left?.CompareTo(right) < 0;
...
public static bool TryParse(string? s, IFormatProvider? formatProvider,
                            out url url) => TryParse(s, out url);
...
{
    public override
        url Read(ref Utf8JsonReader reader, Type typeToConvert,
                 JsonSerializerOptions options) => From(reader.GetString());

    public override void Write(Utf8JsonWriter writer, url value,
...
{
    public override url Read(ref Utf8JsonReader reader, type typeToConvert,
                             Jso options)
    {
...
public static class urlEfCoreExtensions {
  public static void
  Configureurl<TEntity>(this ModelBuilder modelBuilder,
  ...
  public static void Configureurl<TEntity>(this EntityTypeBuilder<TEntity> entityBuilder,
  Expression<Func<TEntity, url>> propertyExpression)
      where TEntity : class => entityBuilder.Property(propertyExpression).HasConversion<url.EfCoreValueConverter>(); }
#endif
