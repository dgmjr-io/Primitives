      ...
      IFormattable
{
    ...
    public const string Description =
        "an email address in the form of *user@domain.ext*";
    public const string UriPrefix = "mailto:";
    public const string UriPattern = $"{UriPrefix}{{0}}";

    public readonly Uri Uri =>
        IsEmpty ? null! : new(Format(UriPattern, ToString()));
  ...
  /// </summary>
  static string IStringWithRegexValueObject<EmailAddress>.Description =>
      Description;
  ...
  /// </summary>
  static string IStringWithRegexValueObject<EmailAddress>.RegexString =>
      RegexString;
#else
    readonly Regx IStringWithRegexValueObject<EmailAddress>.Regex() => Regex();

    readonly string IStringWithRegexValueObject<EmailAddress>.RegexString =>
        RegexString;
    readonly string IStringWithRegexValueObject<EmailAddress>.Description =>
        Description;
    ...
    public static EmailAddress Empty =>
        From(EmptyValueString) with { OriginalString = EmptyValueString };
  ...
  public static partial Regex Regex();
    ...
    public static Validation Validate(string? s) =>
        ...
        : s.Length == 0 ? Validation.Invalid("Email address cannot be empty.")
        : !Regex().IsMatch(s) ? Validation.Invalid("Email address is not valid.")
                              : Validation.Ok;

    public static bool operator !=(EmailAddress? a, EmailAddress? b) =>
        a.HasValue && b.HasValue &&
        CompareOrdinal(a.Value.Value, b.Value.Value) == 0;
    ...
    public readonly string
        ToString(string? format, IFormatProvider? formatProvider) => ToString();
    ...
    /// <returns>A bool.</returns>
    public static bool
    TryParse(string? s, out EmailAddress email) => TryParse(s, null, out email);
    ...
    operator string?(EmailAddress? addr) => addr.HasValue ? addr.Value.Value
                                                          : string.Empty;

    public readonly int
        CompareTo(EmailAddress? other) => CompareTo(other?.Value ?? string.Empty);
...
public static class EmailAddressEfCoreExtensions
{
    ...
    public static void ConfigureEmailAddress<TEntity>(
        ...
        where TEntity : class => modelBuilder.Entity<TEntity>().ConfigureEmailAddress(propertyExpression);
    ...
    /// <param name="propertyExpression">The property expression.</param>
    public static void ConfigureEmailAddress<TEntity>(this EntityTypeBuilder<TEntity> entityBuilder,
    Expression<Func<TEntity, EmailAddress>> propertyExpression)
        where TEntity : class => entityBuilder.Property(propertyExpression).HasConversion<EmailAddress.EfCoreValueConverter>();
}
