    ...
    where TSelf : PrimitivesTests<TPrimitive, TSelf>
{
    ...
    public void Empty_Primitive_Is_Empty()
    {
        ...
        Logger.LogInformation(
            $"Testing that empty primitive {emptyPrimitive} IsEmpty == true");
    ...
    public virtual void
    Valid_Primitive_Is_Valid_And_Not_Empty(string validValue)
    {
        ...
        primitive.Value.Trim().ToLowerInvariant().Should().Contain(
            validValue.Trim().ToLowerInvariant());
    }

    // [Theory]
    // [MemberData(nameof(InvalidValues))]
    // public virtual void Invalid_Primitive_Is_Invalid_And_Empty(string
    // invalidValue)
    // {
    //     var primitive = TPrimitive.TryParse(invalidValue, null, out var p) ? p
    //     : TPrimitive.Empty; primitive.IsEmpty.Should().BeTrue();
    ...
    [MemberData(nameof(InvalidValues))]
    public virtual void
    Invalid_Primitive_Throws_Exception_On_Parse(string invalidValue)
    {
